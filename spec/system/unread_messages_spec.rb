...
RSpec.describe "Unread messages" do
  ...
  it "shows an unread badge in the sidebar" do
    ...
    expect(page).to have_css("aside span", text: "1")
  end
  ...
  it "shows an unread dot on the conversation and clears it after viewing" do
    ...
    sign_in_and_visit(tenant, conversations_path)
    expect(page).to have_css("span.rounded-full.bg-accent")

    click_link conversation.title
    visit conversations_path
    expect(page).not_to have_css("span.rounded-full.bg-accent")
  end
end
