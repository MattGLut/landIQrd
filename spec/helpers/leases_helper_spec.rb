...
RSpec.describe LeasesHelper, type: :helper do
  describe "#lease_dashboard_tags" do
    ...
    it "returns only the expiring soon tag when there are no open work orders" do
      lease = build(:lease, status: :active, end_date: 2.weeks.from_now.to_date, unit: build(:unit, id: 42))
    ...
    it "returns only the work order tag when the lease is not expiring soon" do
      lease = build(:lease, status: :active, end_date: 6.months.from_now.to_date, unit: build(:unit, id: 42))

      expect(helper.lease_dashboard_tags(lease, work_order_counts: { 42 => 2 })).to eq([
        { label: "Work order", color: :blue }
      ])
    end
